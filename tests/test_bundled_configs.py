...
def test_hayward_aqualogic_config_loads():
    ...
    assert {t.label for t in config.triggers} == {
        ...
        "Minus (wired-local)",
        "Plus (wired-local)",
        "Pool/Spa Mode (wired-local)",
        "Filter (wired-local)",
        "System Off / Service (wired-local)",
        "wired-local Left (tentative)",
        "wired-local Right (tentative)",
        "wired-local (main panel), AUX/valve/heater (subcode)",
        "Valve3 (wired-local)",
        "Valve4/Heater2 (wired-local)",
        "Heater1 (wired-local)",
